...
import { MM_API_KEY } from './../../ApiKeys';
...
export function fetchSongId(title) {
  return function (dispatch) {
    ...
    const myApiCall = 'http://api.musixmatch.com/ws/1.1/track.search?&q_track=' + title + '&page_size=1&s_track_rating=desc&apikey=' + MM_API_KEY;
    return fetch(myApiCall).then(
...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch('http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=' + musicMatchId + '&apikey=' + MM_API_KEY).then(
    response => response.json(),
    error => console.log('An error occurred.', error)
  ).then(function(json) {
